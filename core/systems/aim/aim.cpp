...
vec3 CalcAngle(const vec3& src, const vec3& dst)
{
	...
	angle.x = atan2(-delta.z, hyp) * (180.0 / M_PI);
...
auto aimbot(RenderData data, Camera& camera, uintptr_t ptr) -> void
{
	...
	vec3 old_angle = vec3();
	vec3 headeyes = recode::driver.read<vec3>(ptr + 0x1fc4); // headeyes bone position

	vec3 view_angle = recode::driver.read<vec3>(ptr + 0x2600); // m_ammoPoolCapacity - 0x14
	...
	recode::driver.write<vec2>(ptr + 0x2600, vec2(view_angle.x + delta.x, view_angle.y + delta.y));
}